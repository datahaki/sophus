...
public class SnCurveDecimationTest extends TestCase {
  public void testGeodesic() {
    Tensor p0 = VectorNorm2.NORMALIZE.apply(Tensors.vector(1, 0, 0));
    Tensor p1 = VectorNorm2.NORMALIZE.apply(Tensors.vector(0.5, 0.5, 0));
    Tensor p2 = VectorNorm2.NORMALIZE.apply(Tensors.vector(0, 1, 0));
  ...
  public void testTriangle() throws ClassNotFoundException, IOException {
    Tensor p0 = VectorNorm2.NORMALIZE.apply(Tensors.vector(1, 0, 0));
    Tensor p1 = VectorNorm2.NORMALIZE.apply(Tensors.vector(0.5, 0.5, 0.8));
    Tensor p2 = VectorNorm2.NORMALIZE.apply(Tensors.vector(0, 1, 0));
    Tensor tensor = Tensors.of(p0, p1, p2);
