...
public class ClothoidTest extends TestCase {
  public void testLength() throws ClassNotFoundException, IOException {
    ...
    for (int count = 0; count < 100; ++count) {
      ...
      Clothoid curve = Serialization.copy(Se2ClothoidBuilder.INSTANCE.curve(p, q));
  ...
  public void testLengthZero() {
    ...
    for (int count = 0; count < 100; ++count) {
      Tensor p = RandomVariate.of(distribution, 3);
      Clothoid clothoid = Se2ClothoidBuilder.INSTANCE.curve(p, p);
  ...
  public void testCurvatureZero() {
    ...
    for (int count = 0; count < 100; ++count) {
      Tensor p = RandomVariate.of(distribution, 3);
      LagrangeQuadraticD curvature = Se2ClothoidBuilder.INSTANCE.curve(p, p).curvature();
      curvature.head();
  ...
  public void testLengthMid() {
    ...
    for (int count = 0; count < 100; ++count) {
      Tensor p = RandomVariate.of(distribution, 3);
      Tensor q = RandomVariate.of(distribution, 3);
      Clothoid clothoid = Se2ClothoidBuilder.INSTANCE.curve(p, q);
      Tensor m = clothoid.apply(RationalScalar.HALF);
      Scalar l1 = Se2ClothoidBuilder.INSTANCE.curve(p, m).length();
      Scalar l2 = Se2ClothoidBuilder.INSTANCE.curve(m, q).length();
      Chop._01.requireClose(l1, l2);
    }
  }

  public void testQuantity() {
    Clothoid clothoid = Se2ClothoidBuilder.INSTANCE.curve(Tensors.fromString("{1[m], 2[m], 3}"), Tensors.fromString("{7[m], -2[m], 4}"));
  ...
  public void testSimple() throws ClassNotFoundException, IOException {
    ...
    LagrangeQuadraticD clothoidTerminalRatio = Se2ClothoidBuilder.INSTANCE.curve(p, q).curvature();
    Scalar head = clothoidTerminalRatio.head();
    LagrangeQuadraticD clothoidCurvature = Serialization.copy(Se2ClothoidBuilder.INSTANCE.curve(p, q).curvature());
  ...
  public void testStraight() {
    ...
    LagrangeQuadraticD clothoidCurvature = Se2ClothoidBuilder.INSTANCE.curve(p, q).curvature();
  ...
  public void testAngles() {
    ...
    for (Tensor angle : angles) {
      Clothoid clothoid = Se2ClothoidBuilder.INSTANCE.curve(pxy.append(angle), qxy.append(angle));
  ...
  public void testLeft() {
    LagrangeQuadraticD headTailInterface = //
        Se2ClothoidBuilder.INSTANCE.curve(Tensors.vector(0, 1, 0), Tensors.vector(2, 2, 0)).curvature();
  ...
  public void testRight() {
    LagrangeQuadraticD headTailInterface = //
        Se2ClothoidBuilder.INSTANCE.curve(Tensors.vector(0, 1, 0), Tensors.vector(2, 0, 0)).curvature();
  ...
  public void testFixedLeftUnit() {
    LagrangeQuadraticD headTailInterface = Se2ClothoidBuilder.INSTANCE.curve( //
  ...
  public void testOfLeftUnit() {
    LagrangeQuadraticD clothoidTerminalRatio = Se2ClothoidBuilder.INSTANCE.curve( //
        Tensors.fromString("{0[m], 1[m], 0}"), Tensors.fromString("{2[m], 2[m], 0}")).curvature();
