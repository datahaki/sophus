...
public class ClothoidTest extends TestCase {
  public void testLength() throws ClassNotFoundException, IOException {
    ...
    for (int count = 0; count < 100; ++count) {
      ...
      Clothoid curve = Serialization.copy(Se2Clothoid.of(p, q));
  ...
  public void testLengthZero() {
    ...
    for (int count = 0; count < 100; ++count) {
      Tensor p = RandomVariate.of(distribution, 3);
      Clothoid clothoid = Se2Clothoid.of(p, p);
  ...
  public void testCurvatureZero() {
    ...
    for (int count = 0; count < 100; ++count) {
      Tensor p = RandomVariate.of(distribution, 3);
      LagrangeQuadraticD curvature = Se2Clothoid.of(p, p).curvature();
  ...
  public void testCurvature() throws ClassNotFoundException, IOException {
    ...
    for (int count = 0; count < 100; ++count) {
      Tensor p = RandomVariate.of(distribution, 3);
      Tensor q = RandomVariate.of(distribution, 3);
      HeadTailInterface headTailInterface = Se2Clothoid.of(p, q).curvature();
  ...
  public void testLengthMid() {
    ...
    for (int count = 0; count < 100; ++count) {
      ...
      Scalar l1 = Se2Clothoid.of(p, m).length();
      Scalar l2 = Se2Clothoid.of(m, q).length();
      Chop._01.requireClose(l1, l2);
    }
  }

  public void testQuantity() {
    Clothoid clothoid = Se2Clothoid.of(Tensors.fromString("{1[m], 2[m], 3}"), Tensors.fromString("{7[m], -2[m], 4}"));
  ...
  public void testSimple() throws ClassNotFoundException, IOException {
    ...
    HeadTailInterface clothoidTerminalRatio = Se2Clothoid.of(p, q).curvature();
    Scalar head = clothoidTerminalRatio.head();
    LagrangeQuadraticD clothoidCurvature = Serialization.copy(Se2Clothoid.of(p, q).curvature());
  ...
  public void testStraight() {
    ...
    LagrangeQuadraticD clothoidCurvature = Se2Clothoid.of(p, q).curvature();
  ...
  public void testAlmostStraight() {
    ...
    HeadTailInterface headTailInterface1 = Se2Clothoid.of(p, q).curvature();
    Scalar head = headTailInterface1.head();
    LagrangeQuadraticD lagrangeQuadraticD = Se2Clothoid.of(p, q).curvature();
  ...
  public void testAngles() {
    ...
    for (Tensor angle : angles) {
      Clothoid clothoid = Se2Clothoid.of(pxy.append(angle), qxy.append(angle));
  ...
  public void testLeft() {
    HeadTailInterface headTailInterface = //
        Se2Clothoid.of(Tensors.vector(0, 1, 0), Tensors.vector(2, 2, 0)).curvature();
  ...
  public void testRight() {
    HeadTailInterface headTailInterface = //
        Se2Clothoid.of(Tensors.vector(0, 1, 0), Tensors.vector(2, 0, 0)).curvature();
  ...
  public void testFixedLeftUnit() {
    HeadTailInterface headTailInterface = Se2Clothoid.of( //
  ...
  public void testOfLeftUnit() {
    HeadTailInterface clothoidTerminalRatio = Se2Clothoid.of( //
        Tensors.fromString("{0[m], 1[m], 0}"), Tensors.fromString("{2[m], 2[m], 0}")).curvature();
